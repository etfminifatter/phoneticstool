...
## 基本功能
- 上传Excel文件，智能识别包含单词的列（支持"词"、"单词"、"word"等多种列名）
- 自动生成英式和美式音标
...
### 步骤式设计
整个页面采用明确的步骤式设计，让用户清晰了解操作流程：

1. **文件上传区域**：拖放或选择上传Excel文件
2. **列选择与预览区**：选择并确认要处理的单词列
3. **处理控制区**：执行处理、查看结果和下载
...
3. 在列选择区选择包含单词的列，并预览确认内容
4. 点击"开始处理"按钮，系统会处理并生成音标
5. 处理完成后，点击"下载结果"按钮获取带音标的Excel文件

### 支持的Excel格式
- 文件必须包含一个带有单词/短语的列（支持多种列名，如"词"、"单词"、"word"、"term"、"vocabulary"等）
- 如果找不到标准列名，系统会智能识别或提示选择
- 支持.xlsx文件格式，最大10MB
...
### 3. 短语分词机制
- **检测空格**：系统检查输入中是否包含空格
- **执行分词**：所有含空格的输入都会被空格分割成多个单词，没有任何例外情况
- **单独处理**：每个分词后的单词分别查询音标
...
### 4. 列名识别机制
- **记住上次选择**：首先尝试使用localStorage中记录的上次用户选择的列名
- **预设名称**：如果没有历史记录，则在扩展的预设列名列表中查找匹配（"词"、"单词"、"word"、"term"、"vocabulary"、"english"、"生词"等多种常见列名）
- **模糊匹配**：如果找不到精确匹配，则查找包含关键词的列名
- **单列识别**：如果Excel文件只有一列，则自动使用该列
- **用户选择**：如以上方法都失败，提供界面让用户手动选择列，并记住此选择以便下次使用
...
### Q: 短语如何处理？
A: 所有包含空格的短语（如"look after"、"a lot"等）都会被分解为独立单词，分别查询音标后再合并结果。这种处理方式能最大限度提高查询成功率，确保即使整个短语在词典中不存在，也能获得各个单词的音标。

### Q: 如果Excel中列名不是"词"怎么办？
A: 系统会智能识别多种常见列名（如"单词"、"word"、"english"、"生词"等各种可能的名称）。系统会记住您上次选择的列名，下次处理同类文件时会优先使用。如果无法自动识别，会提供界面让您手动选择正确的列。
